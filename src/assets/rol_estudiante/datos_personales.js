...
(function () {
    ...
    root.innerHTML = SECCIONES.map(sec => {
        // Conteo de cambios
        const cambios = sec.campos.reduce((n, f) => n + (same(perfilBase[f.k], perfilEnviado[f.k]) ? 0 : 1), 0);
        ...
        <button type="button"
          ...
          <div class="min-w-0">
            ...
            <p class="text-xs text-gray-500">
              <b>${cambios}</b> actualizados · ${total - cambios} sin cambios
            </p>
          </div>
            ...
            return `
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-2 px-4 py-3">
                  ...
                  <dd class="sm:col-span-2 text-sm text-gray-900 flex flex-col sm:flex-row sm:items-center sm:gap-3">
                    ...
                    <span class="text-xs text-gray-500">Registro base: ${fmtValor(baseV, f)}</span>
                    <span class="sm:ml-auto mt-1 sm:mt-0">${chipEstado(cambio)}</span>
                  </dd>
                </div>
              `;
        }).join('')}
