...
document.addEventListener('DOMContentLoaded', () => {
  ...
  `;
    ...
    <div class="mt-3 grid gap-6 md:grid-cols-2 xl:grid-cols-4">
      <div>
        <h6 class="text-sm font-semibold text-gray-800 mb-1">Anexos prioritarios</h6>
        <ul class="text-sm text-gray-700 space-y-1">
          <li>
            <b>Documento de identificación</b>
            ${aDoc?.file_url ? `<a href="#" data-download="${aDoc.file_url}" data-filename="documento_identidad.pdf" class="ml-2 text-xs text-blue-700 hover:underline">descargar</a>` : ''}
          </li>
          <li>
            <b>Certificado SaberPro/TyT</b>
            ${aCert?.file_url ? `<a href="#" data-download="${aCert.file_url}" data-filename="certificado_saberpro.pdf" class="ml-2 text-xs text-blue-700 hover:underline">descargar</a>` : ''}
          </li>
          <li><b>Código Saber:</b> ${val(aCode)}</li>
        </ul>
      </div>

      <div>
        <h6 class="text-sm font-semibold text-gray-800 mb-1">Contacto y vínculos</h6>
        <ul class="text-sm text-gray-700 space-y-1">
          <li><b>Teléfono:</b> ${val(get(schema.contacto, 'telefono'))}</li>
          <li><b>Correo personal:</b> ${val(get(schema.contacto, 'correo_personal'))}</li>
          <li><b>Departamento:</b> ${val(get(schema.contacto, 'departamento'))}</li>
          <li><b>Ciudad:</b> ${val(get(schema.contacto, 'ciudad'))}</li>
          <li><b>Dirección:</b> ${val(get(schema.contacto, 'direccion'))}</li>
        </ul>
      </div>

      <div>
        <h6 class="text-sm font-semibold text-gray-800 mb-1">Vínculos con la institución</h6>
        <ul class="text-sm text-gray-700 space-y-1">
          <li><b>Hijo(a) de funcionario:</b> ${val(get(schema.institucion, 'hijo_func'))}</li>
          <li><b>Hijo(a) de docente:</b> ${val(get(schema.institucion, 'hijo_doc'))}</li>
          <li><b>Funcionario UCMC:</b> ${val(get(schema.institucion, 'es_func'))}</li>
          <li><b>Docente UCMC:</b> ${val(get(schema.institucion, 'es_doc'))}</li>
        </ul>
      </div>

      <div>
        <h6 class="text-sm font-semibold text-gray-800 mb-1">Información adicional</h6>
        <ul class="text-sm text-gray-700 space-y-1">
          <li><b>Título de pregrado:</b> ${val(get(schema.postgrado, 'titulo_pre'))}</li>
          <li><b>Universidad de egreso:</b> ${val(get(schema.postgrado, 'universidad_pre'))}</li>
          <li><b>Fecha de grado:</b> ${val(get(schema.postgrado, 'fecha_grado'))}</li>
        </ul>
      </div>
    </div>
  `;
